...
export class OwnerService {
  ...
  view(id: number): Observable<Owner> {//listar un solo teatro
  ...
  delete(id: number) {
    return this.http.delete<Owner>(`${environment.url_ms_business}/owners/${id}`,
    );
  }

  create(Owner: Owner): Observable<Owner> {
    return this.http.post<Owner>(`${environment.url_ms_business}/owners`, Owner);
  }
  //en la proxima clase colocamos el interceptor

  update(Owner: Owner): Observable<Owner> {
    return this.http.put<Owner>(`${environment.url_ms_business}/owners/${Owner.id}`, Owner);
  }
}
